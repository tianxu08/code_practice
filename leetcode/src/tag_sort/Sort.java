...
//        System.out.println(Arrays.toString(array));
        task2_moveZero(array);
        System.out.println(Arrays.toString(array));
    }

    // use a pivot(0), [ > 0, left, right, ==0]
    ...
    // [s, f)  useless
    // [f, n)  to explore
    public static int[] task2_moveZero(int[] array) {
        ...
        // after the above loop, the [0, s) are elements > 0, put the following 0 to [s, n)
    ...
    public static void merge(int[] array, int[] helper, int left, int mid, int right) {
        // copy the merging part to helper array
        ...
        while (leftIdx <= mid && rightIdx <= right) {
            if (helper[leftIdx] <= helper[rightIdx]) {
                ...
                // pick the helper[rIdx]
                array[left ++] = helper[rightIdx ++];
            }
        }

        while (leftIdx <= mid) {
            // !!! here, pick element from helper to array
            array[left++] = helper[leftIdx++];
        }
    }
    ...
    public static int partition(int[] array, int left, int right) {
        ...
        int leftP = left;
        int rightP = right - 1;
        while (leftP <= rightP) { // !!! leftBound <= rightBound
            if (array[leftP] < pivot) {
                leftP ++;
            } else if (array[rightP] >= pivot) {
                rightP --;
            } else {
    ...
    public static void test5() {
        ...
        int[] output = task2_moveZero(array);
